...
$seo = SeoPages::find()->where(['name'=>'discounts'])->one();
$this->title = $seo->seo_title;
$this->registerMetaTag(["name"=>"description","content"=>$seo->seo_descr]);

$this->params['breadcrumbs'][] = "Скидки";

$news = "";
foreach ($model as $new) {
   $news = $news."<div class='news-block'>";
   $news = $news.Html::a("<div class='news-img col-lg-4 col-md-4 col-sm-4 col-xs-4'><img style='max-height:130px;' class='img-responsive' src='".$new->img."'/></div>",['site/discounts','id'=>$new->id]);
   $news = $news."<div style='border-left:1px solid #eee;margin-left:170px; text-align:left'>";
   $news = $news.Html::a($new->title,['site/discounts','id'=>$new->id],['class'=>'news-title']);
